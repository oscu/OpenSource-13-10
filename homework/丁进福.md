...
## 其他比较 ##
 ...
 序号   指标                             MogileFS：      
  ...
  1    系统简洁性                 简洁：只有两角色(tracker、storage)              
  
  2    系统性能                   很高（没有使用数据库，文件同步直接点对点，不经过tracker中转）   
 
  3    系统稳定性                 高（C语言开发，支持高并发和高负载）                 
 
  4    RAID方式                   分组（组内冗余），灵活性较大                      
  
  5    通讯协议                   专有协议，下载文件支持HTTP         
 
  6    技术文档                   较详细                      
 
  7    文件附加属性（meta data）  支持                      
  ...
  1    系统简洁性                 一般：三角色（tracker、storage和存储文件信息的mysql.db）
  
  2    系统性能                   高（使用mysql存储文件索引等信息，文件同步通过tracker调度和中转）
 
  3    系统稳定性                 一般（Per1语言开发，高并发和高负载支持一般） 
